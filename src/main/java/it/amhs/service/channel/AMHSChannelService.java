...
public class AMHSChannelService {
    ...
    private AMHSChannel resolveMissingChannel(String normalizedChannelName) {
        if (!databaseEnabled) {
            return buildEnabledChannel(normalizedChannelName);
        }

        if (DEFAULT_CHANNEL_NAME.equals(normalizedChannelName)) {
            return channelRepository.save(buildEnabledChannel(DEFAULT_CHANNEL_NAME));
        }
    ...
    private String normalizeChannelName(String channelName) {
        return StringUtils.hasText(channelName) ? channelName.trim().toUpperCase() : DEFAULT_CHANNEL_NAME;
    }
