...
| LV-ENV-03 | Transfer-envelope uses high-tag-number unknown extension (`[31]` and above) observed in gateway-specific BER emitters. | Preserve unknown extension tag and payload octets exactly in `unknownExtensions` for forward compatibility replay. | `P1BerMessageParserTest.shouldPreserveHighTagOperationalUnknownEnvelopeExtensionOctets` |
| LV-ENV-04 | Transfer-envelope unknown extension payload carries peer-defined criticality markers not modeled by this profile. | Treat payload as opaque unknown-extension value; do not reinterpret criticality metadata; preserve bytes unchanged. | `P1BerMessageParserTest.shouldTreatLegacyCriticalityLikeOpaqueUnknownExtensionPayload` |
